...
DevDoctor is a read-only CLI for catching common local, repository, environment, cache, HTTP URL, database, queue, Docker, Composer, Git, Node/frontend, Python, Go, Rust, and CI problems before they turn into manual debugging sessions.
...
```bash
php devdoctor app:build devdoctor.phar --build-version=1.17.0 --no-interaction
...
- Health aggregates local project diagnostics across `presets`, `env`, `cache`, `http`, `php`, `node`, `laravel`, `composer`, `db`, `queue`, `git`, `docker`, and `security`; it adds `frontend`, `python`, `go`, and `rust` automatically when matching presets are detected. Add `--include-ports` to include common local port checks.
...
`v1.17.0` ships `devdoctor rust` with static diagnostics for Cargo manifests, lockfiles, workspaces, toolchains, dependency sources, and release profiles. The remaining ecosystem commands (`java`, `cpp`, `dotnet`, `symfony`, and `web`) are planned as separate releases so `ci --modules=<name>` only accepts them once their analyzers exist.
...
The CI aggregator runs `env`, `php`, `node`, `laravel`, `composer`, `git`, and `docker` by default. It adds `frontend`, `python`, `go`, and `rust` automatically when matching presets are detected. `ports` and `security` are excluded by default because they can depend on local machine state or intentionally present local files.
...
```yaml
- uses: rtcoder/devdoctor@v1.17.0
  with:
    version: v1.17.0
    format: sarif
```
