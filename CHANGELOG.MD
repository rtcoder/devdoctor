...
- Expanded the command documentation page with every available command option parsed from CLI signatures.
- Updated command metadata JSON to include arguments, options, defaults, repeatable flags, and raw signatures.
- Reworked the command page layout so commands and copyable examples render one below another instead of compressed grid cards.

## v1.47.0 - 2026-06-08

- Included MCP diagnostics in the `security` CI profile.
- Added test coverage proving MCP supply-chain findings affect security-profile CI exit codes.
- Updated README and CI documentation so security-oriented runs explicitly cover agent configuration risks.

## v1.46.0 - 2026-06-08

- Added MCP project policy flags: `--disallow-remote`, `--allow-command`, and `--deny-command`.
- Added MCP policy issue codes for disallowed remote transports and stdio command allow/deny-list violations.
- Updated CLI, README, docs, issue code catalog, and tests for policy-driven MCP diagnostics.

## v1.45.0 - 2026-06-08

- Added MCP supply-chain diagnostics for unpinned package runner targets such as `npx`, `pnpm`, `yarn`, `bunx`, and `uvx`.
- Added MCP Docker image hygiene checks for implicit tags and `latest` image tags in `docker run` stdio server definitions.
- Updated MCP docs, issue code catalog entries, and fixture-based tests for package and Docker pinning.

## v1.44.0 - 2026-06-08

- Expanded MCP config discovery for Claude, Cline, Continue, Roo, and Windsurf project config files.
- Added support for nested `mcp.servers` and `mcp.mcpServers` config sections.
- Updated docs and tests for common agent client MCP config discovery.

## v1.43.0 - 2026-06-08

- Expanded `devdoctor mcp` with agent configuration hardening checks.
- Added support for `.agents/mcp.json` and `.codex/mcp.json`.
- Added MCP issue codes for inline secrets, missing env references, insecure remote URLs, and risky stdio commands.

## v1.42.0 - 2026-06-08

- Added `devdoctor mcp` for read-only Model Context Protocol configuration diagnostics.
- Added typed MCP DTOs and transport enum support for `stdio`, `sse`, and `http` server definitions.
- Added MCP preset detection, command catalog metadata, issue code catalog entries, and fixture-based tests without starting MCP servers or making network calls.
...
- The helper refreshes `composer.lock` by default and supports `--no-lock` for dry text-only version rewrites.
