...
final class IssueSuggestionCatalog
{
    ...
    public static function for(string $code, array $context = []): array
    {
        ...
        return match ($code) {
            ...
            'DD_NODE_BINARY_MISSING' => self::suggest('Install Node.js or add it to PATH before running Node diagnostics.'),
            'DD_NODE_LOCK_MISSING' => self::suggest('Generate and commit the intended package manager lockfile.'),
            'DD_NODE_MODULES_MISSING' => self::suggest('Install dependencies with the package manager selected by the project lockfile.'),
            'DD_NODE_MULTIPLE_LOCKFILES' => self::suggest('Keep only the lockfile for the package manager the project actually uses.'),
            'DD_NODE_PACKAGE_JSON_INVALID' => self::suggest('Fix package.json syntax before running package manager commands.'),
            'DD_NODE_PACKAGE_MANAGER_MISMATCH' => self::suggest('Align packageManager with the committed lockfile.'),
            'DD_NODE_SCRIPT_RISKY' => self::suggest('Review the script and replace remote shell execution with a safer, auditable command.'),
            'DD_NODE_VERSION_FILE_CONFLICT' => self::suggest('Choose one Node.js version policy and align package.json, .nvmrc, and .node-version.'),
            'DD_NODE_VERSION_MISMATCH' => self::suggest('Use a Node.js runtime that satisfies the project requirement.'),
            'DD_LARAVEL_APP_KEY_MISSING' => self::suggest('Generate and set APP_KEY before running the application.'),
            'DD_LARAVEL_APP_URL_DEFAULT' => self::suggest('Set APP_URL to the canonical local, staging, or production URL for this project.'),
            'DD_LARAVEL_CONFIG_CACHED' => self::suggest('Rebuild Laravel config cache after changing environment or config files.'),
            'DD_LARAVEL_DIRECTORY_MISSING' => self::suggest('Create the expected Laravel runtime directory.'),
            'DD_LARAVEL_DIRECTORY_NOT_WRITABLE' => self::suggest('Adjust ownership or permissions so the current user can write Laravel runtime files.'),
            'DD_LARAVEL_ENV_MISSING' => self::suggest('Create .env from .env.example and fill required Laravel values.'),
            'DD_LARAVEL_PROD_DEBUG' => self::suggest('Disable APP_DEBUG before using production environment settings.'),
            'DD_SECURITY_DOCKER_PRIVILEGED' => self::suggest('Avoid privileged containers unless the service explicitly requires host-level access.'),
            'DD_SECURITY_DOCKER_SOCKET_MOUNT' => self::suggest('Avoid mounting the Docker socket into containers unless this is an intentional trusted control plane.'),
            'DD_SECURITY_ENV_NOT_IGNORED' => self::suggest('Add .env patterns to .gitignore to reduce accidental secret commits.'),
            'DD_SECURITY_RISKY_COMPOSER_SCRIPT' => self::suggest('Review the Composer script and replace remote shell execution with a safer install step.'),
            'DD_SECURITY_RISKY_PACKAGE_SCRIPT' => self::suggest('Review the package script and replace remote shell execution with a safer install step.'),
            'DD_SECURITY_SECRET_IN_EXAMPLE' => self::suggest('Replace the value with a placeholder and rotate the credential if it was real.'),
            'DD_SECURITY_SECRET_PATTERN' => self::suggest('Move hard-coded secrets into a secret manager or local environment file and rotate exposed values.'),
