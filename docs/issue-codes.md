...
- `DD_MCP_COMMAND_DENIED` - An MCP stdio command is denied by project policy. Introduced in `1.46.0`; status `active`.
- `DD_MCP_COMMAND_NOT_ALLOWED` - An MCP stdio command is not in the project allow-list. Introduced in `1.46.0`; status `active`.
...
- `DD_MCP_REMOTE_DISALLOWED` - An MCP remote server is disallowed by project policy. Introduced in `1.46.0`; status `active`.
